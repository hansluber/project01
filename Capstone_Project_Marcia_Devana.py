...
# Mencari karyawan berdasarkan ID, Nomor Induk Karyawan, atau Nama
    def search_employee(self):
        print("\n\nKetik 'cancel' untuk kembali ke main menu.\n")
        search_type = input("Cari berdasarkan (1) Nama atau (2) Nomor Induk Karyawan: ")

        if search_type == '1':
            nama = input("Masukkan Nama: ")
            if nama.lower() == 'cancel':
                return
            results = [emp for emp in self.employees.values() if emp['Nama'].lower() == nama.lower()]
        elif search_type == '2':
            nomor_induk = input("Masukkan Nomor Induk Karyawan: ")
            if nomor_induk.lower() == 'cancel':
                return
            results = [emp for emp in self.employees.values() if emp['Nomor Induk Karyawan'] == nomor_induk]
        else:
        ...
        if results:
            for emp in results:
                ...
                print(f"Departemen: {emp['Departemen']}")
                print(f"Nomor Induk Karyawan: {emp['Nomor Induk Karyawan']}")
                print(f"e-mail: {emp['e-mail']}")
                print(f"Status Perkawinan: {emp['Status Perkawinan']}")
        else:
